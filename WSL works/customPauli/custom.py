...
class CustomPauliFeatureMap(NLocal):
    ...
    def __init__(
        ...
        alpha: Union[float, Parameter, ParameterVector] = 2.0,
        ...
        self._alpha = alpha
        self._parameter_vector_index = 0
    ...
    @property
    def alpha(self) -> Union[float,Parameter,ParameterVector]:
    ...
    @alpha.setter
    def alpha(self, alpha: Union[float, Parameter, ParameterVector]) -> None:
    ...
    def entanglement_blocks(self):
        l = [self.pauli_block(pauli) for pauli in self._paulis]
        print([li.draw() for li in l])
        print("-------------------------------------------------------------")
        return l
    ...
    def pauli_evolution(self, pauli_string, time):
        ...
        basis_change(evo)
        cx_chain(evo)
        if not isinstance(self.alpha, ParameterVector):
            evo.p(self.alpha * time, indices[-1])
        else:
            evo.p(self.alpha.params[self._parameter_vector_index]*time,indices[-1])
            self._parameter_vector_index += 1
